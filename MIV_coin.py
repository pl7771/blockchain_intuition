...
class Blockchain:
    ...
    # adding transactions to transactionslist
    ...
    # adding nodes with different addresses
    def add_node(self, address):
        parsed_url = urlparse(address)
        self.nodes.add(parsed_url.netloc)
    
    # replace all chains by longest chain in network
...
def add_transaction():
    ...
    responce = {'message': f'This transaction will be added to the next block {index} '}
...
def connect_node():
    ...
    if nodes is None:
        return 'No node connected', 400
    for node in nodes:
        blockchain.add_node(node)
    responce = {'message': 'Nodes connected, network nodes: ',
